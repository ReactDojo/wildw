    ...
    return (
      <TouchableHighlight underlayColor='rgb(11,148,150)' onPress={() => this.openModal(item)} style={styles.listview} >
        <View>
          ...
          <Text numberOfLines={1} style={styles.listcontenttext_re}>{item.story}</Text>
...
render() {
  return (
    <View style={{ flex: 1}}>
      <FlatList
        style={{ flex: 1 }}
        data={this.props.redeemed_offers}
...
function mapStateToProps(state, props) {
  return {
    redeemed_offers: state.OfferReducer.redeemed_offers,
    qr_code: state.OfferReducer.qr_code,
    isFetching: state.OfferReducer.isFetching
  }
}
export default connect(mapStateToProps, { fetchAvailableOffers })(MyOfferReddemList);
